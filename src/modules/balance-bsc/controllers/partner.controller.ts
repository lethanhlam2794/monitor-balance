    ...
    try {
      ...
      if (partners.length === 0) {
        return {
          success: true,
          message:
            '📋 **Danh sách Partners**\n\n❌ Chưa có partner nào được cấu hình.',
        };
      }

      let message = '📋 **Danh sách Partners**\n\n';
      ...
      partners.forEach((partner, index) => {
        ...
        message += `• Tên: \`${escapeMarkdownV2(partner.name)}\`\n`;
        message += `• Địa chỉ: \`${escapeMarkdownV2(addressShort)}\`\n`;
        message += `• Token: ${escapeMarkdownV2(partner.tokenSymbol)}\n`;
        message += `• Chain: ${escapeMarkdownV2(partner.chainId.toString())}\n`;
        if (partner.description) {
          message += `• Mô tả: ${escapeMarkdownV2(partner.description)}\n`;
        }
        message += '\n';
      });
      ...
      return {
        success: false,
        message: '❌ Lỗi khi lấy danh sách partners!',
      };
    }
  }
...
**Lưu ý:** Partner sẽ được ẩn (soft delete) chứ không xóa hoàn toàn khỏi database`;
  ...
  async createPartner(partnerData: {
    ...
    try {
      ...
      return {
        success: true,
        message: `✅ Partner "${escapeMarkdownV2(partnerData.displayName)}" đã được tạo thành công\\!`,
      };
    } catch (error) {
      this.logger.error('Error in createPartner:', error);
      return {
        success: false,
        message: '❌ Có lỗi xảy ra khi tạo partner\\!',
      };
    }
  }
}
