...
export class DiscordWebhookService {
  ...
  /**
   * Send API error notification to Discord
   */
  async sendApiErrorNotification(errorDetails: {
    ...
    try {
      ...
      // Add user details if less than 10 users
  ...
  /**
   * Send API recovery notification
   */
  ...
  /**
   * Send custom audit log to Discord (raw error/message)
   */
  async auditWebhook(
    title: string,
    description: string,
